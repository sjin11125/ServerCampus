...
public class GameDB : IGameDB
{
    ...
    public async Task<ErrorCode> DeleteItem(string email, int itemId)        //게임 아이템 지우기
    {
   ...
   public async Task<(ErrorCode,List<UserItem>)> GetAllItems(string email)         //유저 아이템 모두 불러오기
    ...
    public async Task<(ErrorCode, UserItem)> GetItem(string email, int itemId) //유저 아이템  불러오기
    {
        var items = await queryFactory.Query("itemdata").Where("Email", email).Where("Id", itemId).FirstOrDefaultAsync<UserItem>();
    ...
    public async Task<(ErrorCode, string )> ReadMail(string email, int mailId)      //메일 읽기
    {
        var content = await queryFactory.Query("mail").Where("Email", email).Where("Id", mailId).Select("Content").FirstOrDefaultAsync<string>();
        ...
        await queryFactory.Query("mail").Where("Email", email).Where("Id", mailId).UpdateAsync(new { isRead=true});     //읽음 처리함
        return (ErrorCode.None, content);

    }

    public async Task<(ErrorCode,List<MailItem>)> GetMailItem(string email,int mailId)      //유저 메일 아이템 정보 받기
    {
        var items = await queryFactory.Query("mailitem").Where("Email", email).Where("Id", mailId).GetAsync<MailItem>();
        if (items is null)
    ...
    public async Task<ErrorCode> ReceiveMailItem(string email,int mailId)                //유저 메일 아이템 받기 처리
    {
        var result = await queryFactory.Query("mail").Where("Email", email).Where("Id", mailId).Select("isGet").FirstOrDefaultAsync<string>();

        if (result is null)
            return ErrorCode.GetMailItemFail;

        await queryFactory.Query("mail").Where("Email", email).Where("Id", mailId).UpdateAsync(new { isGet = true });     //받음 처리함
    ...
    public async Task<ErrorCode> InsertMailItems(string email, List<UserItem> items, int itemId) //메일 아이템 테이블에 아이템 넣기
    {
    ...
    public async Task<(ErrorCode, int)> Attendance(int AccountId)          //출석 확인
    {
        var result = await queryFactory.Query("gamedata").Where("AccountId", AccountId).Select("Attendance", "AttendanceCount").FirstOrDefaultAsync<AttendanceInfo>();     //사용자 게임정보 중 날짜를 불러옴
        ...
        {
           await queryFactory.Query("gamedata").Where("AccountId", AccountId).UpdateAsync(new { AttendanceCount = 1, Attendance = DateTime.Today });
        ...
        {
            await queryFactory.Query("gamedata").Where("AccountId", AccountId).UpdateAsync(new { AttendanceCount = result.AttendanceCount+1, Attendance = DateTime.Today });
